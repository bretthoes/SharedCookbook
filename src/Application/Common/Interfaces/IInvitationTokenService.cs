...
public interface IInvitationTokenService
{
    ...
    /// </summary>
    /// <returns>
    /// A <see cref="GeneratedLinkCode"/> containing:
    /// <list type="bullet">
    /// <item><description><c>InviteToken</c> — Base64URL-encoded secret to send to the client.</description></item>
    /// <item><description><c>Stored</c> — <see cref="InvitationCodeHash"/> with <c>Hash</c> (SHA-256 of salt||code) and <c>Salt</c> to persist in the database.</description></item>
    /// </list>
    /// </returns>
    /// <remarks>
    ...
    /// <param name="stored">The stored <see cref="InvitationCodeHash"/> (hash and salt) from the invitation row.</param>
    ...
    /// </remarks>
    bool Verify(string inviteToken, InvitationCodeHash stored);
    ...
    /// <returns>
    /// An <see cref="InvitationLink"/> with the numeric invitation id and the Base64URL token,
    ...
    /// </remarks>
    InvitationLink? Parse(string raw);
}

public sealed record InvitationCodeHash(byte[] Hash, byte[] Salt);
public sealed record GeneratedLinkCode(string InviteToken, InvitationCodeHash Stored);
public sealed record InvitationLink(int InvitationId, string InviteToken);
